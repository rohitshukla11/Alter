...
type Props = {
  ...
  children: ReactNode;
  /** Step 2 (publish) headline — default demo emphasizes Web3 Architect. */
  createHeadline?: string;
  createSubline?: string;
};

export function Shell({ step, onStep, children, createHeadline, createSubline }: Props) {
  const isCreateStep = step === 2;
  const title = isCreateStep && createHeadline ? createHeadline : "Create your AI advisor";
  const sub =
    isCreateStep && createSubline
      ? createSubline
    ...
    <div className="fixed inset-0 z-0 flex h-[100dvh] w-full bg-void text-primary">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="shrink-0 border-b border-dim px-6 py-5 md:px-10">
          <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-accent">Alter</p>
          <h1 className="mt-1 font-display text-2xl font-extrabold leading-tight text-primary md:text-3xl">{title}</h1>
          <p className="mt-2 max-w-xl font-mono text-[13px] leading-relaxed text-secondary">{sub}</p>
        </header>
